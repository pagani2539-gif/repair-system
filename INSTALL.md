...
3.  **ตั้งค่า Environment (`.env`)**:
    ...
    cp .env.example .env
    ```
    เปิดไฟล์ `.env` แล้วตั้งค่าอย่างน้อย `JWT_SECRET` (ค่าสุ่มยาว) และ
    `SEED_ADMIN_PASSWORD` (รหัสผ่าน admin เริ่มต้น)
...
4.  **ฐานข้อมูล (อัตโนมัติ)**:
    ไม่ต้องสร้างฐานข้อมูลเอง — ระบบจะสร้างไฟล์ `repair_system.db`, รัน migration และ
    seed บัญชี admin เริ่มต้นให้อัตโนมัติตอน Server สตาร์ทครั้งแรก
    *(ถ้าต้องการสร้างล่วงหน้าสามารถรัน `npm run init-db` ได้ แต่ไม่จำเป็น)*
